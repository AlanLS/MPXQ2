 ...
 * @author Shai Almog
 */
public class ConvInboxCellRenderer implements ListCellRenderer, CellRenderer
{
	 ...
	 */
	public ConvInboxCellRenderer(Component selected, Component unselected)
	{
	...
	private Component[] initRenderer(Component r)
	{
		...
		{
			final Vector selectedVector = new Vector();
	...
	{
		final Button c = lastClickedComponent;
	 ...
	 */
	public ConvInboxCellRenderer(Component odd, Component oddUnselected, Component even, Component evenUnselected)
	{
	...
	{
		final Component[] result = new Component[v.size()];
	...
	private void findComponentsOfInterest(Component cmp, Vector dest)
	{
		...
		{
			final Container c = (Container) cmp;
		...
		else if (((cmp instanceof Label) || (cmp instanceof TextArea)) && (cmp.getName() != null))
		{
			dest.addElement(cmp);
		}
		return;
	...
	public Component getCellRendererComponent(Component list, Object model, Object value, int index, boolean isSelected)
	{
		...
		if (isSelected && list.hasFocus())
		{
			...
			if (value instanceof Hashtable)
			{
				...
				final Boolean enabled = (Boolean) h.get(ENABLED);
				if (enabled != null)
				{
				...
				{
					final String currentName = entries[iter].getName();
					Object val;
		...
		if (cmp instanceof Label)
		{
			...
			if (label.shouldTickerStart() && Display.getInstance().shouldRenderSelection())
			{
				if (!label.isTickerRunning())
				{
					...
					if (parentList != null)
					{
						final Form f = parentList.getComponentForm();
						if (f != null)
						{
	...
	class Monitor implements ActionListener, Animation
	{
		...
		public boolean animate()
		{
			if (parentList != null)
			{
				...
				if (pendingAnimations != null)
				{
					final int s = pendingAnimations.size();
					for (int iter = 0; iter < s; iter++)
					{
						final Image i = (Image) pendingAnimations.elementAt(iter);
		...
		public void actionPerformed(ActionEvent evt)
		{
			...
			if (parentList instanceof List)
			{
				final Object selection = ((List) parentList).getSelectedItem();
				if (selection instanceof Hashtable)
				{
					final Hashtable h = (Hashtable) selection;
					final Command cmd = (Command) h.get("$navigation");
					if (cmd != null)
					{
					...
					for (int iter = 0; iter < selectedEntries.length; iter++)
					{
						...
						{
							final boolean sel = !isSelectedValue(h.get(selectedEntries[iter].getName()));
							if (h.get(SELECT_ALL_FLAG) != null)
							{
								...
								final int count = ((List) parentList).getModel().getSize();
								final String selectionVal = "" + sel;
								for (int x = 0; x < count; x++)
								{
									final Object o = ((List) parentList).getModel().getItemAt(x);
							...
							else
							{
								if (selectAllChecked)
								{
									...
									final Hashtable selAll = (Hashtable) ((List) parentList).getModel().getItemAt(selectAllOffset);
