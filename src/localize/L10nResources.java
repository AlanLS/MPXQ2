...
public class L10nResources
{
	...
	public String getString(String key, String[] params)
	{
		...
		else
		{
			...
			{
				final int index = result.indexOf("{" + i + "}");
				if (index == -1)
				{
	...
	{
		Reader reader = null;
		final String resourePath = getResourcePath(locale);
		final InputStream stream = getClass().getResourceAsStream(resourePath);
		if (stream == null)
		{
		...
		try
		{
			...
			while ((letter = reader.read()) != -1)
			{
				...
				if (letter == '\n')
				{
					...
					final String key = line.substring(0x00, separator);
					final String value = line.substring(++separator, line.length());
